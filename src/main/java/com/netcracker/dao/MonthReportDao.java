...
public interface MonthReportDao {
    ...
    String GET_MONTH_REPORT_BY_FAMILY_ACCOUNT_ID = "SELECT OBJ.OBJECT_ID AS MONTH_REPORT_ID, TOTALINCOME.VALUE AS INCOME, " +
            ...
            " ATTRIBUTES BALANCE, ATTRIBUTES DATE_FROM, ATTRIBUTES DATE_TO, OBJREFERENCE OBJ " +
            " WHERE OBJ.ATTR_ID = 11  AND OBJ.REFERENCE = ? " +
            " AND TOTALINCOME.OBJECT_ID = OBJ.OBJECT_ID AND TOTALINCOME.ATTR_ID = 12 /*TOTALINCOME*/ " +
            " AND TOTALEXPENSE.OBJECT_ID = OBJ.OBJECT_ID AND TOTALEXPENSE.ATTR_ID = 13 /*TOTALEXPENSE*/ " +
            " AND BALANCE.OBJECT_ID = OBJ.OBJECT_ID AND BALANCE.ATTR_ID = 14 /*BALANCE*/ " +
            " AND DATE_FROM.OBJECT_ID = OBJ.OBJECT_ID AND  DATE_FROM.ATTR_ID = 15 AND DATE_FROM.DATE_VALUE = ? " +
            " AND DATE_TO.OBJECT_ID = OBJ.OBJECT_ID AND DATE_TO.ATTR_ID = 16 AND DATE_TO.DATE_VALUE = ?";

    String GET_MONTH_REPORT_BY_PERSONAL_ACCOUNT_ID = "SELECT OBJ.OBJECT_ID AS MONTH_REPORT_ID, TOTALINCOME.VALUE AS INCOME, " +
            "  TOTALEXPENSE.VALUE AS EXPENSE, BALANCE.VALUE AS BALANCE, " +
            "  DATE_TO.DATE_VALUE AS DATE_TO, DATE_FROM.DATE_VALUE AS DATE_FROM " +
            "  FROM ATTRIBUTES TOTALINCOME, ATTRIBUTES TOTALEXPENSE, " +
            "  ATTRIBUTES BALANCE, ATTRIBUTES DATE_FROM, ATTRIBUTES DATE_TO, OBJREFERENCE OBJ " +
            "  WHERE OBJ.ATTR_ID = 10  AND OBJ.REFERENCE = ? " +
            "  AND TOTALINCOME.OBJECT_ID = OBJ.OBJECT_ID AND TOTALINCOME.ATTR_ID = 12 /*TOTALINCOME*/ " +
            "  AND TOTALEXPENSE.OBJECT_ID = OBJ.OBJECT_ID AND TOTALEXPENSE.ATTR_ID = 13 /*TOTALEXPENSE*/ " +
            "  AND BALANCE.OBJECT_ID = OBJ.OBJECT_ID AND BALANCE.ATTR_ID = 14 /*BALANCE*/ " +
            "  AND DATE_FROM.OBJECT_ID = OBJ.OBJECT_ID AND  DATE_FROM.ATTR_ID = 15 AND DATE_FROM.DATE_VALUE = ? /* date from */ " +
            "  AND DATE_TO.OBJECT_ID = OBJ.OBJECT_ID AND DATE_TO.ATTR_ID = 16 AND DATE_TO.DATE_VALUE = ? /* date to*/ ";
    ...
    String GET_CATEGORY_INCOME_PERSONAL_REPORT_BY_ID = "SELECT  SUMM.VALUE AS AMOUNT, CATEGORY.LIST_VALUE_ID AS CATEGORY" +
            "  FROM ATTRIBUTES SUMM,  ATTRIBUTES CATEGORY, OBJREFERENCE REF" +
            "  WHERE REF.ATTR_ID = 17 AND REF.REFERENCE = ?" +
            "  AND SUMM.OBJECT_ID = REF.OBJECT_ID AND SUMM.ATTR_ID = 21 /* amount*/" +
            "  AND CATEGORY.OBJECT_ID = REF.OBJECT_ID AND CATEGORY.ATTR_ID = 20 /* category*/";

    String GET_CATEGORY_INCOME_FAMILY_REPORT_BY_ID = "SELECT SUMM.VALUE AS AMOUNT, " +
            "CATEGORY.LIST_VALUE_ID AS CATEGORY, CONSUMER.REFERENCE AS USER_ID" +
            "  FROM ATTRIBUTES SUMM,  ATTRIBUTES CATEGORY, OBJREFERENCE REF, OBJREFERENCE CONSUMER" +
            "  WHERE REF.ATTR_ID = 18  AND REF.REFERENCE = ?" +
            "  AND SUMM.OBJECT_ID = REF.OBJECT_ID AND SUMM.ATTR_ID = 21 /* amount*/" +
            "  AND CATEGORY.OBJECT_ID = REF.OBJECT_ID AND CATEGORY.ATTR_ID = 20" +
            "  AND CONSUMER.ATTR_ID = 19 AND CONSUMER.OBJECT_ID = REF.OBJECT_ID";

    String GET_CATEGORY_EXPENSE_PERSONAL_REPORT_BY_ID = "SELECT SUMM.VALUE AS AMOUNT, CATEGORY.LIST_VALUE_ID AS CATEGORY" +
            " FROM ATTRIBUTES SUMM,  ATTRIBUTES CATEGORY, OBJREFERENCE REF" +
            " WHERE REF.ATTR_ID = 22  AND REF.REFERENCE = ?" +
            " AND SUMM.OBJECT_ID = REF.OBJECT_ID AND SUMM.ATTR_ID = 26 /* amount*/" +
            " AND CATEGORY.OBJECT_ID = REF.OBJECT_ID AND CATEGORY.ATTR_ID = 25 /* category*/";

    String GET_CATEGORY_EXPENSE_FAMILY_REPORT_BY_ID = "SELECT  SUMM.VALUE AS AMOUNT, " +
            " CATEGORY.LIST_VALUE_ID AS CATEGORY, CONSUMER.REFERENCE AS USER_ID" +
            "  FROM ATTRIBUTES SUMM,  ATTRIBUTES CATEGORY, OBJREFERENCE REF, OBJREFERENCE CONSUMER" +
            "  WHERE  REF.ATTR_ID = 23  AND REF.REFERENCE = ?" +
            "  AND SUMM.OBJECT_ID = REF.OBJECT_ID AND SUMM.ATTR_ID = 26 /* amount*/" +
            "  AND CATEGORY.OBJECT_ID = REF.OBJECT_ID AND CATEGORY.ATTR_ID = 25 /* category*/" +
            "  AND CONSUMER.ATTR_ID = 24 AND CONSUMER.OBJECT_ID = REF.OBJECT_ID";

    String GET_ALL_PERSONAL_REPORTS = "SELECT OBJ.OBJECT_ID AS MONTH_REPORT_ID, TOTALINCOME.VALUE AS INCOME, " +
            "  TOTALEXPENSE.VALUE AS EXPENSE, BALANCE.VALUE AS BALANCE, " +
            "  DATE_TO.DATE_VALUE AS DATE_TO, DATE_FROM.DATE_VALUE AS DATE_FROM " +
            "  FROM ATTRIBUTES TOTALINCOME, ATTRIBUTES TOTALEXPENSE, " +
            "  ATTRIBUTES BALANCE, ATTRIBUTES DATE_FROM, ATTRIBUTES DATE_TO, OBJREFERENCE OBJ " +
            ...
            "  AND TOTALEXPENSE.OBJECT_ID = OBJ.OBJECT_ID AND TOTALEXPENSE.ATTR_ID = 13 /*TOTALEXPENSE*/ " +
            "  AND BALANCE.OBJECT_ID = OBJ.OBJECT_ID AND BALANCE.ATTR_ID = 14 /*BALANCE*/ " +
            "  AND DATE_FROM.OBJECT_ID = OBJ.OBJECT_ID AND  DATE_FROM.ATTR_ID = 15  /* date from */ " +
            "  AND DATE_TO.OBJECT_ID = OBJ.OBJECT_ID AND DATE_TO.ATTR_ID = 16 /* date to*/";

    String GET_ALL_FAMILY_REPORTS = "SELECT OBJ.OBJECT_ID AS MONTH_REPORT_ID, TOTALINCOME.VALUE AS INCOME, " +
            "  TOTALEXPENSE.VALUE AS EXPENSE, BALANCE.VALUE AS BALANCE, " +
            "  DATE_TO.DATE_VALUE AS DATE_TO, DATE_FROM.DATE_VALUE AS DATE_FROM " +
            "  FROM ATTRIBUTES TOTALINCOME, ATTRIBUTES TOTALEXPENSE, " +
            "  ATTRIBUTES BALANCE, ATTRIBUTES DATE_FROM, ATTRIBUTES DATE_TO, OBJREFERENCE OBJ " +
    ...
    String GET_FULL_PERSONAL_REPORTS = "SELECT OBJ.OBJECT_ID AS MONTH_REPORT_ID, TOTALINCOME.VALUE AS INCOME, " +
            "  TOTALEXPENSE.VALUE AS EXPENSE, BALANCE.VALUE AS BALANCE, " +
            "  DATE_TO.DATE_VALUE AS DATE_TO, DATE_FROM.DATE_VALUE AS DATE_FROM " +
            "  FROM ATTRIBUTES TOTALINCOME, ATTRIBUTES TOTALEXPENSE, " +
            "  ATTRIBUTES BALANCE, ATTRIBUTES DATE_FROM, ATTRIBUTES DATE_TO, OBJREFERENCE OBJ  " +
            ...
            "  AND TOTALEXPENSE.OBJECT_ID = OBJ.OBJECT_ID AND TOTALEXPENSE.ATTR_ID = 13 /*TOTALEXPENSE*/ " +
            "  AND BALANCE.OBJECT_ID = OBJ.OBJECT_ID AND BALANCE.ATTR_ID = 14 /*BALANCE*/ " +
            "  AND DATE_FROM.OBJECT_ID = OBJ.OBJECT_ID AND  DATE_FROM.ATTR_ID = 15 AND EXTRACT(DAY FROM DATE_FROM.DATE_VALUE) LIKE '1'  /* date from */ " +
            "  AND DATE_TO.OBJECT_ID = OBJ.OBJECT_ID AND DATE_TO.ATTR_ID = 16 AND EXTRACT(DAY FROM DATE_TO.DATE_VALUE) LIKE '1' /* date to*/ " +
            "  ORDER BY DATE_FROM";

    String GET_FULL_FAMILY_REPORTS = "SELECT OBJ.OBJECT_ID AS MONTH_REPORT_ID, TOTALINCOME.VALUE AS INCOME, " +
            "  TOTALEXPENSE.VALUE AS EXPENSE, BALANCE.VALUE AS BALANCE, " +
            "  DATE_FROM.DATE_VALUE AS DATE_FROM, DATE_TO.DATE_VALUE AS DATE_TO " +
            "  FROM ATTRIBUTES TOTALINCOME, ATTRIBUTES TOTALEXPENSE, " +
            "  ATTRIBUTES BALANCE, ATTRIBUTES DATE_FROM, ATTRIBUTES DATE_TO, OBJREFERENCE OBJ " +
            ...
            "  AND TOTALEXPENSE.OBJECT_ID = OBJ.OBJECT_ID AND TOTALEXPENSE.ATTR_ID = 13 /*TOTALEXPENSE*/ " +
            "  AND BALANCE.OBJECT_ID = OBJ.OBJECT_ID AND BALANCE.ATTR_ID = 14 /*BALANCE*/ " +
            "  AND DATE_FROM.OBJECT_ID = OBJ.OBJECT_ID AND  DATE_FROM.ATTR_ID = 15 AND EXTRACT(DAY FROM DATE_FROM.DATE_VALUE) LIKE '1' " +
            "  AND DATE_TO.OBJECT_ID = OBJ.OBJECT_ID AND DATE_TO.ATTR_ID = 16 AND EXTRACT(DAY FROM DATE_TO.DATE_VALUE) LIKE '1' " +
            "  ORDER BY DATE_FROM";
}
