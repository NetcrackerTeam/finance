...
public interface FamilyAccountDebitDao {
    ...
    String GET_PARTICIPANTS = "SELECT\n" +
            ...
            "    NVL((SELECT SUM(INCOM.VALUE) FROM ATTRIBUTES INCOM, ATTRIBUTES DATE_FROM, OBJREFERENCE REF\n" +
            "         WHERE  REF.ATTR_ID = 54 /*  REFERENCE INCOME TO USER */\n" +
            "           AND REF.REFERENCE = DEBIT_TO_USER.REFERENCE\n" +
            "           AND INCOM.OBJECT_ID = REF.OBJECT_ID\n" +
            "           AND INCOM.ATTR_ID = 56\n" +
            "           AND DATE_FROM.OBJECT_ID = REF.OBJECT_ID\n" +
            "           AND DATE_FROM.DATE_VALUE >TRUNC(SYSDATE,'mm')), 0) AS TOTAL_INCOME,\n" +
            "    NVL((SELECT SUM(EXPEN.VALUE) FROM ATTRIBUTES EXPEN, ATTRIBUTES DATE_FROM, OBJREFERENCE REF\n" +
            "         WHERE  REF.ATTR_ID = 49  /*  REFERENCE EXPENSE TO USER */\n" +
            "           AND REF.REFERENCE = DEBIT_TO_USER.REFERENCE\n" +
            "           AND EXPEN.OBJECT_ID = REF.OBJECT_ID\n" +
