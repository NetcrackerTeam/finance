...
public interface FamilyAccountDebitDao {
    ...
    String FIND_FAMILY_ACCOUNT_BY_ID = "SELECT " +
            ...
            +
            "ATTRIBUTES STATUS_USER, OBJREFERENCE DEBIT_TO_USER, OBJREFERENCE USER_TO_PERSONAL  "
            +
            "WHERE DEBIT.OBJECT_ID = ? " +
            "AND AMOUNT.OBJECT_ID = DEBIT.OBJECT_ID " +
            "AND AMOUNT.ATTR_ID = 9 " +
            "AND STATUS_DEBIT.OBJECT_ID = DEBIT.OBJECT_ID " +
            "AND STATUS_DEBIT.ATTR_ID = 69 " +
            "AND DEBIT_TO_USER.ATTR_ID = 2 " +
            "AND DEBIT_TO_USER.REFERENCE = DEBIT.OBJECT_ID " +
            "AND USER_ID.OBJECT_ID = DEBIT_TO_USER.OBJECT_ID " +
            "AND NAME_USER.OBJECT_ID = USER_ID.OBJECT_ID " +
            "AND NAME_USER.ATTR_ID = 5 " +
            "AND EMAIL_USER.OBJECT_ID = USER_ID.OBJECT_ID " +
            "AND EMAIL_USER.ATTR_ID = 3 " +
            "AND PASSWORD_USER.OBJECT_ID = USER_ID.OBJECT_ID " +
            "AND PASSWORD_USER.ATTR_ID = 4 " +
            "AND STATUS_USER.OBJECT_ID = USER_ID.OBJECT_ID " +
            "AND STATUS_USER.ATTR_ID = 6 " +
            "AND USER_TO_PERSONAL.ATTR_ID = 1 " +
    ...
    String GET_PARTICIPANTS = "SELECT "
            ...
            "AND STATUS_USER.OBJECT_ID = USER_ID.OBJECT_ID " +
            "AND STATUS_USER.ATTR_ID = 6 " +
            "AND USER_TO_PERSONAL_DEBIT.ATTR_ID = 1 " +
