...
public interface UserDao {
    ...
    void setPasswordToken(String email, String token);

    User findUserByToken(String token);

    void clearPassToken(BigInteger userId);
    ...
    String GET_USER_BY_LOGIN = "SELECT USERS.NAME OBJ_NAME, " +
            ...
            "  AND STATUS.LIST_VALUE_ID = 39 /* USER IS ACTIVE */ " +
    ...
    String GET_USER_BY_TOKEN = "SELECT USERS.NAME OBJ_NAME, " +
            "       NAME.VALUE NAME,  MAIL.VALUE EMAIL, USERS.OBJECT_ID AS USER_ID, " +
            "       PASSWORD.VALUE AS PASSWORD, STATUS.LIST_VALUE_ID AS IS_ACTIVE, USER_ROLE.LIST_VALUE_ID AS ROLE, " +
            "       (SELECT PER_ACC.REFERENCE FROM OBJREFERENCE PER_ACC WHERE USERS.OBJECT_ID  = PER_ACC.OBJECT_ID AND PER_ACC.ATTR_ID = 1) as PER_DEB_ACC1, " +
            "       (SELECT FAM_ACC.REFERENCE FROM OBJREFERENCE FAM_ACC WHERE USERS.OBJECT_ID  = FAM_ACC.OBJECT_ID AND FAM_ACC.ATTR_ID = 2) as FAM_DEB_ACC1 " +
            "from OBJECTS USERS, ATTRIBUTES MAIL, ATTRIBUTES PASSWORD, ATTRIBUTES NAME, ATTRIBUTES STATUS, ATTRIBUTES USER_ROLE, ATTRIBUTES PASS_TOKEN " +
            "where USERS.OBJECT_ID = MAIL.OBJECT_ID AND MAIL.ATTR_ID = 3 " +
            "  AND USERS.OBJECT_ID = PASSWORD.OBJECT_ID AND PASSWORD.ATTR_ID = 4 " +
            "  AND USERS.OBJECT_ID = PASS_TOKEN.OBJECT_ID AND PASS_TOKEN.ATTR_ID = 73 AND PASS_TOKEN.VALUE = ? " +
            "  AND USERS.OBJECT_ID = NAME.OBJECT_ID AND NAME.ATTR_ID = 5 " +
            "  AND USERS.OBJECT_ID = STATUS.OBJECT_ID AND STATUS.ATTR_ID = 6" +
            "  AND STATUS.LIST_VALUE_ID = 39 /* USER IS ACTIVE */ " +
            "  and USERS.OBJECT_ID = USER_ROLE.OBJECT_ID AND USER_ROLE.ATTR_ID = 71";
    ...
    String UPDATE_TOKEN = "UPDATE ATTRIBUTES SET VALUE = ? WHERE OBJECT_ID = ? AND ATTR_ID = 73";

    String CREATE_USER_TOKEN = "INSERT INTO ATTRIBUTES(ATTR_ID, OBJECT_ID, VALUE) VALUES (73, ?, ?)";

    String CLEAR_PASS_TOKEN = "UPDATE ATTRIBUTES SET VALUE = NULL WHERE OBJECT_ID = ? AND ATTR_ID = 73";

    String IF_TOKEN_ATTR_EXISTS = "SELECT COUNT(PASS_TOK_ATTR.OBJECT_ID) FROM ATTRIBUTES PASS_TOK_ATTR, ATTRIBUTES MAIL_ATTR\n" +
            "WHERE PASS_TOK_ATTR.ATTR_ID = 73 /*TOKEN PASS ATTR*/ " +
            "AND PASS_TOK_ATTR.OBJECT_ID = MAIL_ATTR.OBJECT_ID " +
            "AND MAIL_ATTR.VALUE = ? AND MAIL_ATTR.ATTR_ID = 3 /* MAIL*/";
    ...
    String USER_EXIST = "SELECT COUNT(*) " +
            ...
            "AND EMP_STATUS.LIST_VALUE_ID = 39 /* USER IS ACTIVE */ " +
